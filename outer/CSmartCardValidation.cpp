...
CSmartCardValidation::CSmartCardValidation()
    :                                                                         //
      m_pContext(NULL),                                                       //
      ...
      m_Validity(0), m_bCheckExpirationDate(false), m_CardConferenceLevel(0), //
      m_BioUser(0),                                                           //
...
CSmartCardValidation::~CSmartCardValidation() { SAFE_DELETE(m_pSmartCardHandler); }

CValidationResult CSmartCardValidation::ValidateCard(const digicon::CCard Card,
                                                     ...
                                                     CContext *pContext)
{
    ...
    // configuracao.
    CMCA::ECardReader eCardReader = (CMCA::ECardReader)eReader;
    CCardReader *pCardReader = pContext->GetMCA()->GetCardReader(eCardReader);
    ...
    while (TRUE) {
        try {
            ...
            ValResult =
                InternalValidateCard(m_pSmartCardHandler, eReader, nKeyPressed, pContext);
            ...
            if (CardCounter >= 7 || !pCardReader->HasCard()) {
                if (pReaderConfig->GetValidAccessAction().GetUrn().GetAction() == 1) {
                    ...
                    pActionMgr->QueueActions(UrnAction);
                }
                CardCounter = 0;
            ...
            DLOG_INFO("Erro generico na validacao do acesso\n");
            if (pReaderConfig->UseDisplay() && !pActionMgr->IsUsingDisplay()) {
    ...
    try {
        ...
        DLOG_ERROR("Erro atualizando cartao: %s\n", ex.GetMessage());
        if (pReaderConfig->UseDisplay() && !pActionMgr->IsUsingDisplay())
...
CValidationResult
CSmartCardValidation::InternalValidateCard(CSmartCardHandler *pSmartCardHandler,
                                           EReader eReader, BYTE nKeyPressed,
                                           CContext *pContext)
{
    // variaveis utilizadas
    ...
    // Correção por tipo de pessoa - Caso 6725
    int m_iPersonType = m_PeopleKindSit & 0x0F;
    // TODO 2441 - Implementar BDCC ??????
    ...
    if (pReaderConfig->CheckPersonSchedule() != DIR_CONTROL_NO) {
        bool bReaderValidates = false;
        ...
        DLOG_INFO("Direcao checada em AccessDirection: %s\n",
                  (AccessDir == CAccessEvent::AD_None)
                      ? "AD_None"
                      : ((AccessDir == CAccessEvent::AD_In) ? "AD_In" : "AD_Out"));

        // Leitora valida?
        if (AccessDir == CAccessEvent::AD_In &&
            (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH ||
             pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_IN)) {
            bReaderValidates = true;
            DLOG_INFO("Direcao checada em CheckPersonSchedule:%s\n",
                      (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH)
                          ? "DIR_CONTROL_BOTH"
                          : "DIR_CONTROL_IN");
        } else if (AccessDir == CAccessEvent::AD_Out &&
                   (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH ||
                    pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_OUT)) {
            bReaderValidates = true;
            DLOG_INFO("Direcao checada em CheckPersonSchedule:%s\n",
                      (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH)
                          ? "DIR_CONTROL_BOTH"
                          : "DIR_CONTROL_OUT");
        } else if ((AccessDir == CAccessEvent::AD_In ||
                    AccessDir == CAccessEvent::AD_Out) &&
                   (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH)) {
            bReaderValidates = true;
            DLOG_INFO("Direcao checada em CheckPersonSchedule: %s\n", "DIR_CONTROL_BOTH");
        } else {
            bReaderValidates = false;
            DLOG_INFO("Leitora nao valida acesso por faixa horaria!\n");
            DLOG_INFO("Configuracao de CheckPersonalSchedule:%s\n",
                      (pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_BOTH)
                          ? "DIR_CONTROL_BOTH"
                          : ((pReaderConfig->CheckPersonSchedule() == DIR_CONTROL_IN)
                                 ? "DIR_CONTROL_IN"
        ...
        // Se leitora valida, checa se existem os outros controles de permissão
        if (bReaderValidates &&
            (m_bCheckHourlyRangeAccess || m_bCheckHourlyRangeTimeKeeping)) {
            DLOG_INFO("Checando outros controles de permissao...\n");
            DLOG_INFO("CheckHourlyRangeAccess: %d\nCheckHourlyRangeTimeKeeping: %d\n",
                      m_bCheckHourlyRangeAccess, m_bCheckHourlyRangeTimeKeeping);
    ...
    if (CHECK_MIN_STAYTIME(m_CardFlags)) {
        ...
        if (m_AccessDir == CAccessEvent::AD_Out) {
            /* Caso 6725 - Report Sênior | WORD PersonType = 0;
            PersonType = GET_PERSON_TYPE(m_PeopleKindSit);*/
            const BYTE minTime = pReaderConfig->GetMinimumStayTime(m_iPersonType - 1);
            if (minTime) {
    ...
    CValidationResult ValResult;
    bool m_bHasEventBlockType = false;
    bool m_bIsActiveBlockType = false;
    // Tipo de Pessoa esta shiftado dentro do m_PersonKindSit -> Tipo Físico
    // (Pessoa – 4 bits) ; Situação (Lib/Bloq - 4 bits)
    /* Caso Mantis: [3705] - Solicitar Urna ao final da validade para
     * Configuração de Bloqueio */
    // A configuração será utilizada somente quando o controle de tipo de pessoa
    // estiver configurado como bloqueado no sentido do acesso.
    if (!CheckPersonType(pSmartCardHandler,
                         pReaderConfig->GetPersonsAllowOut())) { // Este Metodo
                                                                 // vai consumir
                                                                 // o CardType
        if (pReaderConfig->GetBlockPersonTypeValidation(m_iPersonType - 1) == 0) {
            DLOG_INFO("Default Bloqueado\n");

            DLOG_INFO("Acesso Negado por Tipo de Pessoa 0x0%X\n", m_iPersonType);
            ValResult.SetEvent(CEvent::ACCESS_DENIED_PERSON_TYPE_EXIT);
            ValResult.GetEvent().SetId(CEvent::ACCESS_DENIED_PERSON_TYPE_EXIT);
            m_bHasEventBlockType = true;
        } else if (!CheckExpirationDate(
                       ValResult, pSmartCardHandler,
                       &ResultEvent)) { // como o dispositivo não possui acesso
                                        // à data de validade, deverá obedecer
            // conforme a opção, sendo valor 0 ou 1
            // bloquear imediatamente
            if (pReaderConfig->GetBlockPersonTypeValidation(m_iPersonType - 1) == 1) {
                DLOG_INFO("Bloqueado por Validade do Cartão Expirado\n");

                DLOG_INFO("Acesso Negado por Validade 0x0%X\n", m_iPersonType);
                ValResult.SetEvent(CEvent::ACCESS_DENIED_VALIDITY);
                ValResult.GetEvent().SetId(CEvent::ACCESS_DENIED_VALIDITY);
                m_bHasEventBlockType = true;
            } // contém a data de validade no mapa smartcard, as configurações 1
            // e 2 terão o mesmo efeito, pois tanto online como offline será
            // possível consultar a data e realizar o controle
            else if (pReaderConfig->GetBlockPersonTypeValidation(m_iPersonType - 1) ==
                     2) {
                if (!m_pContext->IsOnline()) // Funciona
                {
                    m_bIsActiveBlockType = true;
                    DLOG_INFO("Offline Permitir\n");
                } // offline
            }     // igual á 2
        }         // fora da validade
    }             // Check PersonTypeDir2
    ...
    // Para não alterar o evento quando for para a condição
    // BlockTypeMotherFucker
    if (!m_bHasEventBlockType) {
        ValResult.SetEvent(ResultEvent);
    }
...
void CSmartCardValidation::SetUseField(BYTE FieldId, BYTE bUse)
{
    m_FieldInUse[FieldId - 1] = bUse;
}

void CSmartCardValidation::DefineInUseFields(const CReaderConfig *pCardReaderConfig)
{
    for (int i = 0; i < 7; i++) {
...
void CSmartCardValidation::ReadCardFlags(CSmartCardHandler *pSmartCardHandler,
...
void CSmartCardValidation::UpdateSmartCard(CSmartCardHandler *pSmartCardHandler,
                                           CAccessEvent *pEvent, short AccessCreditRange,
...
bool CSmartCardValidation::IsInHourlyRange(WORD Time, const BYTE *HourlyRange) const
{
    BYTE range[] =                                       //
...
void CSmartCardValidation::CheckMessages(CSmartCardHandler *pSmartCardHandler)
{
    m_Messages = 0;
    ...
    if (iDateSize > 0) {
        ...
        DLOG_INFO("Show Always: %s\n", bShowMessage ? "yes" : "no");
    } else {
        DLOG_INFO("No MessageExhibitionDate field in the map!\n");
    }
    ...
    if (bShowMessage) {
        ...
        if (MessageSize > 0)
            ...
            DLOG_INFO("No Message field in the map!\n");
...
bool CSmartCardValidation::CheckPermission(CSmartCardHandler *pSmartCardHandler,
                                           EReader eReader, WORD *retPermissionId) const
{
...
bool CSmartCardValidation::CheckPermissionRestrictedArea(
...
bool CSmartCardValidation::CheckSituation(CSmartCardHandler *pSmartCardHandler) const
{
    DLOG_INFO("CheckSituation: (0x%02X)\n", m_PeopleKindSit);
    return GET_SITUATION(m_PeopleKindSit) == 10;
}

bool CSmartCardValidation::CheckValidity(CSmartCardHandler *pSmartCardHandler,
...
bool CSmartCardValidation::CheckLevel(CSmartCardHandler *pSmartCardHandler,
                                      CAccessEvent *pEvent,
...
bool CSmartCardValidation::CheckRemoval(CSmartCardHandler *pSmartCardHandler,
                                        CAccessEvent *pEvent) const
{
...
void CSmartCardValidation::GetCreditRange(CSmartCardHandler *pSmartCardHandler,
                                          time_t DayTimeInMinutes, bool bCheckCredits,
...
short CSmartCardValidation::CheckCredit(CSmartCardHandler *pSmartCardHandler,
                                        short *pRangeNumber)
{
...
bool CSmartCardValidation::CheckAntiPassBack(CSmartCardHandler *pSmartCardHandler,
                                             CAccessEvent *pEvent,
                                             const CFirmwareProperties *pProperties,
                                             const CReaderConfig *pReaderConfig)
{
...
bool CSmartCardValidation::CheckPersonHourlyRange(
...
bool CSmartCardValidation::CheckPermissionHourlyRange(
...
bool CSmartCardValidation::CheckPersonType(CSmartCardHandler *pSmartCardHandler,
...
bool CSmartCardValidation::IsMasterCard() const
{
 ...
 */
bool CSmartCardValidation::CheckTimeOutAntiPassBack(CSmartCardHandler *pSmartCardHandler,
                                                    const CReaderConfig *pReaderConfig)
{
...
bool CSmartCardValidation::CheckLastPassDate(CSmartCardHandler *pSmartCardHandler)
{
...
bool CSmartCardValidation::CheckLunchInterval(CSmartCardHandler *pSmartCardHandler,
...
bool CSmartCardValidation::CheckInterJourney(CSmartCardHandler *pSmartCardHandler,
                                             EReader eReader) const
{
...
bool CSmartCardValidation::CheckMinimumStayTime(CSmartCardHandler *pSmartCardHandler,
                                                EReader eReader,
...
// [Mantis - Caso 3705] -> Origem do metodo criado a partir do caso
bool CSmartCardValidation::CheckExpirationDate(CValidationResult &valRes,
                                               CSmartCardHandler *pSmartCardHandler,
                                               CAccessEvent *pEvent) const
{
    DLOG_INFO("Data do Acesso: %ld\n", m_Now);
    time_t pDateTimeExp = 0;
    BYTE m_byDateExpCard[6];
    BYTE m_byDateNow[6];
    //
    if (!CheckValidity(pSmartCardHandler, pEvent)) { // pDateTimeExp = GetValidityDate();
        pDateTimeExp = pEvent->GetValidity();
        // Se nao tem data de validade deu erro
        if (pDateTimeExp == 0) {
            return false;
        }

        // Converte a Data Atual em Bytes para um Buffer
        CFirmwareUtil::GetDateTimeBytes(m_Now, m_byDateNow);
        WORD m_wDateNow =
            CFirmwareUtil::GetDate(m_byDateNow[0], m_byDateNow[1], m_byDateNow[2]);
        // Converte a Data de Validade em Bytes para um Buffer
        CFirmwareUtil::GetDateTimeBytes(pDateTimeExp, m_byDateExpCard);
        WORD m_wValidityDate = CFirmwareUtil::GetDate(
            m_byDateExpCard[0], m_byDateExpCard[1], m_byDateExpCard[2]);

        // O controle deve considerar apenas se a data do acesso (dia/mes/ano)
        // coincide com a data da validade, descartando o horário.
        if (m_wValidityDate <= m_wDateNow) {
            DLOG_INFO("Data de Validade Expirado!\n");
            // valRes.GetEvent().SetValidity(pDateTimeExp); // Validade do
            // cartao
            return false;
        }
    }
...
bool CSmartCardValidation::LunchCreditControl(CSmartCardHandler *pSmartCardHandler,
                                              EReader eReader,
...
bool CSmartCardValidation::ReadPassword(CSmartCardHandler *pSmartCardHandler)
{
...
bool CSmartCardValidation::ReadLunchIntervalInfo(CSmartCardHandler *pSmartCardHandler,
                                                 BYTE *Range,
                                                 time_t &LunchIntervalStart) const
{
    int iFieldSize = m_pContext->GetProcessSmartCard()->GetDateTimeField(
...
void CSmartCardValidation::SetBackupList(CValBackup *pBackupList)
{
